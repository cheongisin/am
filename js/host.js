...
app.innerHTML = `
  ...
  <div class="topbar"><div class="topbar-inner">  
    ...
    </div>  <div class="grid cols2" style="margin-top:12px">  
  <div class="card">  
    <h3>배정/공개 현황</h3>  
  ...
  </div>`;  app.querySelector('#undoBtn').onclick=()=>{
const ok=undo(game);
if(ok){
...
return <div style="display:flex;justify-content:space-between;gap:10px;padding:6px 0;border-bottom:1px solid rgba(255,255,255,.06)">   <div>${p.name}${p.alive?'':' <span class="muted">(사망)</span>'}</div>   <div class="muted small">${r}${pub}</div>   </div>;
}).join('');
...
return   <div style="display:flex;flex-direction:column;gap:12px">   ${buildTimerPanel()}   ${buildPhasePanel()}   </div>  ;
}
...
return   <div>   <h4 style="margin:0 0 6px">타이머</h4>   <div class="actions">   <span class="badge">현재 ${timerText}</span>   </div>   <div class="grid cols2" style="margin-top:8px">   <div><label>분</label><input id="timerMin" type="number" min="0" value="3"></div>   <div><label>초</label><input id="timerSec" type="number" min="0" max="59" value="0"></div>   </div>   <div class="actions" style="margin-top:8px">   <button id="timerStart" ${disabled}>시작</button>   <button id="timerPause" ${disabled || !running ? 'disabled' : ''}>일시정지</button>   <button id="timerResume" ${disabled || !paused ? 'disabled' : ''}>재개</button>   <button id="timerStop" ${disabled}>리셋</button>   </div>   <div class="actions" style="margin-top:6px">   <button class="timerPreset" data-sec="300" ${disabled}>낮 5분</button>   <button class="timerPreset" data-sec="120" ${disabled}>투표 2분</button>   </div>   <p class="muted small">밤은 무한대로 표시되며 스킵 가능합니다.</p>   </div>  ;
}
...
return   <div class="grid cols2">   <div>   ${sel('마피아 공격', nightDraft.mafiaId, 'mafiaTarget', false)}   ${sel('의사 보호', nightDraft.doctorId, 'doctorTarget', true)}   ${sel('경찰 조사', nightDraft.policeId, 'policeTarget', true)}   </div>   <div>   ${reporterBlock()}   ${sel('테러리스트 지목', nightDraft.terroristId, 'terroristTarget', true)}   </div>   </div>   <div class="actions" style="margin-top:10px"><button class="primary" id="nightResolve" ${disabled}>밤 확정 → DAY</button></div>  ;
}
if(game.phase===PHASE.DAY){
return   <p class="muted">낮 토론</p>   <div class="actions">   <button class="primary" id="toVote" ${disabled}>투표로 이동</button>   <button id="skipDay" ${disabled}>토론 스킵</button>   <button id="manualReveal" ${disabled}>기자 공개(수동)</button>   </div>  ;
}
if(game.phase===PHASE.VOTE){
const alive = game.players.filter(p=>p.alive);
const selected = game.executionTarget ?? alive[0]?.id ?? null;
return    <p class="muted">최후 변론 대상 선택</p>   <label>단두대 대상</label>   <select id="accusedSel" ${disabled}>   ${alive.map(p=><option value="${p.id}" ${p.id===selected?'selected':''}>${p.name}</option>).join('')}   </select>   <div class="actions" style="margin-top:10px">   <button class="primary" id="startDefense" ${disabled}>최후 변론 시작</button>   </div>   ;
}
if(game.phase===PHASE.EXECUTION){
const t=game.executionTarget;
const name = (t==null)? '무효(처형 없음)' : (game.players.find(p=>p.id==t)?.name ?? '-');
return   <p class="muted">투표 진행 중: <b>${name}</b></p>   <div class="actions">   <button class="primary" id="execConfirm" ${disabled}>처형 확정</button>   <button id="execCancel" ${disabled}>무효 → 밤으로</button>   </div>  ;
}
...
return   <label>${title} <span class="muted small">(${actor.name})</span></label>   <select data-key="${key}">   <option value="">${optional?'미사용 / 선택안함':'대상 선택'}</option>   ${opts}   </select>  ;
}
...
return   <label>기자 특보 <span class="muted small">(${actor.name})</span></label>   <div class="actions" style="margin:6px 0">   <input id="repUsed" type="checkbox" style="width:auto" ${checked?'checked':''} ${disabled?'disabled':''}>   <span class="muted small">${game.reporterUsedOnce?'이미 사용함':(disabled?'첫밤 불가':'사용')}</span>   </div>   <select data-key="reporterTarget" ${checked?'':'disabled'}>   <option value="">대상 선택</option>   ${opts}   </select>  ;
}
